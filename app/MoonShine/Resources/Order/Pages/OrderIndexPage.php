...
class OrderIndexPage extends IndexPage
{
    ...
    protected function fields(): iterable
    {
        return [
            ...
            Preview::make('Покупатель', 'username')
                ->changePreview(function ($value, $field) {
                    $order = $field->getData()?->getOriginal();
                    $parts = array_filter([
                        $order?->username,
                        $order?->email,
                        $order?->phone ? format_phone($order->phone) : null,
                    ]);
    ...
    protected function filters(): iterable
    {
        return [
            ...
            Select::make('Услуга', 'product_id')
                ->options(
                    Product::query()
                        ->whereIn('id', Order::distinct()->pluck('product_id'))
                        ->pluck('title', 'id')
                        ->toArray()
                )
                ->nullable()
            ...
            Select::make('Продавец', 'vendor_id')
                ->options(
                    Vendor::query()
                        ->whereIn('id', Order::distinct()->pluck('vendor_id'))
                        ->get()
