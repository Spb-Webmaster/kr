                   ...
                   .then((result) => {
                       ...
                       if (result.errors) {
                           ...
                           hideLoader(() => {
                               ...
                               /** Если в ответе есть редирект — выполняем его **/
                               if (result.response && result.response.redirect) {
                                   window.location.href = result.response.redirect;
                                   return;
                               }

                               /** Если в ответе есть submit_form — обновляем CSRF и сабмитим форму **/
                               if (result.response && result.response.submit_form) {
                                   const form = document.querySelector(result.response.submit_form);
                                   if (form) {
                                       axios.get('/refresh-csrf').then(res => {
                                           form.querySelector('input[name="_token"]').value = res.data.csrf_token;
                                           form.submit();
                                       });
                                   }
                                   return;
